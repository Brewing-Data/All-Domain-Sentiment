...
def remove_unneccessary(inputString):
	...
	inputString1 = re.sub(WEB_URL_REGEX, " URL ", inputString)  # removing url
	inputString2 = re.sub(r"(?:@[\w_]+)", " MENTION ", inputString1)
	inputString3 = re.sub(r"(?:\#+[\w_]+[\w\'_\-]*[\w_]+)", " HASHTAG ", inputString2)
	inputString4 = re.sub("\s+", " ", inputString3)  ## Multiple spaces
	inputString5 = re.sub(r"[^\s]+@([\w_-]+(?:(?:\.[\w_-]+)+))([\w.,@?^=%&:/~+#-]*[\w@?^=%&/~+#-])?", " EMAIL ",
						  inputString4)  # replace email with _email_
	inputString6 = re.sub("\d+", " NUMBER ", inputString5)  # remove numbers
...
def tokenise_and_rem_stop(sentence_series,augmented_words=None):
	...
	tokens_samples = tokens_samples.apply(lambda tokens: [preprocess_word(i) for i in tokens])
